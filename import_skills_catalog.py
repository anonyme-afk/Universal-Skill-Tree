...
def import_catalog():
    ...
    try:
        spec_ust = importlib.util.find_spec("ust")
        if spec_ust and spec_ust.origin:
            ust_dir = os.path.dirname(spec_ust.origin)
            init_file = spec_ust.origin
        else:
            ust_dir = "ust"
    ...
    print(f"Targeting UST directory: {ust_dir}")
    
    if not os.path.exists(init_file):
        os.makedirs(ust_dir, exist_ok=True)
        with open(init_file, "w", encoding="utf-8") as f:
            f.write("_BRANCH_MODULES: dict[str, str] = {}\n")
    ...
    for category, cat_dict in by_cat.items():
        ...
        dir_path = os.path.join(ust_dir, "skills", category)
        ...
        with open(skills_path, "w", encoding="utf-8") as f:
            ...
            f.write(f'"""\n')
            f.write(f'  ⚠️ AUTO-GENERATED FILE ⚠️\n')
            f.write(f'  Do not edit this file directly!\n')
            f.write(f'  Please edit `skills_catalog.py` and run `python import_skills_catalog.py`.\n')
            f.write(f'"""\n')
            ...
            for s in cat_skills:
                ...
                if s.get("requires_confirmation"):
                    f.write(f'    requires_confirmation=True,\n')
                f.write(f')\n')
                ...
                for env in env_vars:
                    ...
                    f.write(f'        raise ValueError("Erreur Plug & Play : clé API manquante ({env}). Ajoutez-la dans .env.ust puis réessayez.")\n')
                ...
                f.write('        raise ImportError("Erreur Plug & Play : package manquant. Demandez a utilisateur de lancer : pip install " + reqs_str)\n')
